$(document).ready(function() {
    $('body').mCustomScrollbar({
        ...
        callbacks:{
            whileScrolling:function(){
                ...
                if ((window.matchMedia('(min-width: 601px)').matches)) {
                    if (this.mcs.top < -300) { $('.scroll-up').fadeIn(); } 
    ...
    $(document).click(function(event) {
        if ($(event.target).closest('#log-in-form').length) { return; }
    ...
    function onResize() {
        ...
        else { $('.page-header, .toast, .floating-toolbar, #log-in-form').removeClass('collapse');  }

        if ((window.matchMedia('(max-width: 1024px)').matches)) { 
            $('nav').addClass('list'); 
        }
        else { $('nav').removeClass('list');  }
    }
